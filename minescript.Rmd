...
free50 <- bind_rows(men_50free_lc,men_50free_sc, women_50free_lc, women_50free_sc)
...
Add Nationality and Some women short course times have period instead of colon
```{r}
...
free400 <- bind_rows(men_400free_lc,men_400free_sc,women_400free_lc, women_400free_sc)
```
...
free800 <- bind_rows(men_800free_lc, men_800free_sc, women_800free_lc, women_800free_sc)
```
...
free1500 <- bind_rows(men_1500free_lc,men_1500free_sc,women_1500free_lc,women_1500free_sc)
```


```{r}
free <- bind_rows(free50,free100,free200,free400,free800,free1500)
```


Add Nationality. Issues with time conversion 
```{r}
...
back100 <- bind_rows(men_100back_lc,men_100back_sc,women_100back_lc,women_100back_sc)
```
...
back <- bind_rows(back100,back200)
```


Issues with time womens short course
```{r}
...
breast100 <- bind_rows(men_100breast_lc,men_100breast_sc,women_100breast_lc,women_100breast_sc)
```
...
Add Nationality. Issues with time in all
```{r}
...
breast <- bind_rows(breast100,breast200)
```


Add Nationality Issues with men SC time
```{r}
...
fly100 <- bind_rows(men_100fly_lc,men_100fly_sc, women_100fly_lc, women_100fly_sc)
```



Add Nationality. women lc issue with time
```{r}
...
fly <- bind_rows(fly100,fly200)
```


men issue with time SC and LC
```{r}
...
im200 <- bind_rows(men_200im_lc,men_200im_sc, women_200im_lc, women_200im_sc)
```

Add Nationality LC. issue with men lc time
```{r}
...
women_400im_sc <- html_table(tables_400im[[4]]) %>%
  select(Time,Swimmer = Name, Nationality,Date,Location) %>%
  mutate(Distance = "400", Stroke = "IM", Format = "sc", Sex = "W")
...
im <- bind_rows(im200,im400)
```

```{r}
record_data <- bind_rows(free,back,breast,fly,im)
record_data$Time[1291] <- 52.07
