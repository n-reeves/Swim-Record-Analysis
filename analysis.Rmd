...
```{r}
sample_1 %>% ggplot() + geom_histogram(aes(x = Number_broken))
sample_2 %>% ggplot() + geom_histogram(aes(x = Number_broken))
```


```{r}
p_both <- df_test %>% mutate(period = ifelse(year<2010, "Before", "After")) %>% ggplot(aes(x=year,y=Number_broken, color = period)) + geom_line()
```

Now Look at just short course and long course resoectively
```{r}
df_test2 <- data %>% filter(year(Date)>1980)

p_lc <- df_test2 %>%filter(Format == "lc") %>% group_by(year = year(Date)) %>% 
  summarize(Number_broken = n()) %>% mutate(period = ifelse(year<2010, "Before", "After")) %>% ggplot(aes(x=year,y=Number_broken, color = period)) + geom_line()
```

```{r}
p_sc <- df_test2 %>%filter(Format == "sc") %>% group_by(year = year(Date)) %>% 
  summarize(Number_broken = n()) %>% mutate(period = ifelse(year<2010, "Before", "After")) %>% ggplot(aes(x=year,y=Number_broken, color = period)) + geom_line()
```

```{r}
df_test2 %>%filter(Format == "lc" & Sex == "M") %>% group_by(year = year(Date)) %>% 
  summarize(Number_broken = n()) %>% mutate(period = ifelse(year<2010, "Before", "After")) %>% ggplot(aes(x=year,y=Number_broken, color = period)) + geom_line()

df_test2 %>%filter(Format == "lc" & Sex == "M"& year(Date) <2020) %>% group_by(year = year(Date)) %>% 
  summarize(Number_broken = n()) %>% mutate(period = ifelse(year<2010, "Before", "After")) %>% group_by(period) %>% summarize(mean = mean(Number_broken))

df_test2 %>%filter(Format == "lc" & Sex == "W") %>% group_by(year = year(Date)) %>% 
  summarize(Number_broken = n()) %>% mutate(period = ifelse(year<2010, "Before", "After")) %>% ggplot(aes(x=year,y=Number_broken, color = period)) + geom_line()

df_test2 %>%filter(Format == "lc" & Sex == "W"& year(Date) <2020) %>% group_by(year = year(Date)) %>% 
  summarize(Number_broken = n()) %>% mutate(period = ifelse(year<2010, "Before", "After")) %>% group_by(period) %>% summarize(mean = mean(Number_broken))
```


Check just sprint events. No Significant differnece between trend postsuit ban in sprint events and trend in aggregate events. Not going to address it in final analysis.
```{r}
df_test2 %>%
